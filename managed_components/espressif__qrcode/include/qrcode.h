...
typedef struct {
	void (*display_func)(
		esp_qrcode_handle_t qrcode); /**< Function called for displaying the QR
										Code after encoding is complete */
...
/**
 * @brief  Encodes the given string into a QR Code and calls the display
 * function
 *
 * @attention 1. Can successfully encode a UTF-8 string of up to 2953 bytes or
 * an alphanumeric string of up to 4296 characters or any digit string of up to
 * 7089 characters
 *
...
#define ESP_QRCODE_CONFIG_DEFAULT()                                            \
	(esp_qrcode_config_t) {                                                    \
		.display_func = esp_qrcode_print_console, .max_qrcode_version = 10,    \
